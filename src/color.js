// based on https://github.com/LeaVerou/contrast-ratio/blob/gh-pages/color.js
...
export class Color {
  constructor(rgba) {
    ...
    rgba = rgba.map(function (a) { return round(a, 3) })
  ...
  distance(color) {
    return Math.pow(this.rgba[0] - color.rgba[0], 2)
      + Math.pow(this.rgba[1] - color.rgba[1], 2)
      + Math.pow(this.rgba[2] - color.rgba[2], 2)
  }
  ...
  contrast(color) {
    ...
    var onBlack = this.overlayOn(BLACK),
      onWhite = this.overlayOn(WHITE),
    ...
    return {
      ratio: round((min + max) / 2, 2),
      error: round((max - min) / 2, 2),
      min: min,
      max: max,
      closest: closest,
...
export const DARK = new Color([66, 66, 66])
export const GRAY = new Color([127.5, 127.5, 127.5])
export const LIGHT = new Color([255, 255, 255])
